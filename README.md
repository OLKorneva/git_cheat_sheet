...
##### 2. Начинающиеся с git вначале
  ...
  - **git push -u origin [branche name]** отправить локальную ветку на удаленный репо, не обязательно находится в ней
  - **git pull** Забрать изменения из удалённого репозитория. Сначала нужно перейти в локальный репозиторий и убедиться, что вы находитесь в правильной ветке, — как правило, это основная ветка main (или master). Затем можно ввести команду.
